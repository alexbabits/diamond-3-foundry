...
* **Problem**: There are too many ways to setup and build on top of the [Diamond proxy pattern](https://eips.ethereum.org/EIPS/eip-2535). When you are just starting out, each tutorial and reference is just different enough to screw up your ability to learn anything. Starting from 0, it takes too long to  comprehend the different combinations and choices of implementations (and then digest all the information and pick one hoping it was correct).
...
* Note: Do not use this code in production, it has NOT been thoroughly tested, and is for educational purposes only.
...
3. How do I verify my diamond contract? IDK `¯\_(ツ)_/¯` I've never built anything worth any value that needed to be verified, and neither will you /s. People say Etherscan is too stupid to view and verify diamonds properly, so [Louper](https://louper.dev/) was built to examine and verify testnet and mainnet diamond protocols.
...
**Inheritance Note**: You cannot inherit an abstract contract that uses storage because that messes up the slot 0 placement where AppStorage should be. I fixed this problem with `AppStorageRoot.sol` but ideally... just don't use any template contracts that have storage.
